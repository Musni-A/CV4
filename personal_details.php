...
if (isset($_POST['add_personal'])) {
    ...
    $profilePhoto = $conn->real_escape_string($_POST['profilePhoto']);
    if ($conn->query("INSERT INTO personal_details (name, email, phone, location, jobTitle, description, profilePhoto) VALUES ('$name', '$email', '$phone', '$location', '$jobTitle', '$description', '$profilePhoto')")) {
...
if (isset($_POST['edit_personal'])) {
    ...
    $jobTitle = $conn->real_escape_string($_POST['jobTitle']);
    $description = $conn->real_escape_string($_POST['description']);
    if ($conn->query("UPDATE personal_details SET name='$name', email='$email', phone='$phone', location='$location', jobTitle='$jobTitle', description='$description' WHERE id=$id")) {
...
<div class="row vh-100">
    ...
    <div class="col-lg-9">
                    ...
                    <table class="table table-bordered align-middle">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td><?= htmlspecialchars($pd['jobTitle']) ?></td>
                                <td><?= htmlspecialchars($pd['description']) ?></td>
                                <td>
                                    <button class="btn btn-info btn-sm" data-bs-toggle="modal" data-bs-target="#viewModal"
                                        onclick="viewDetail('<?= addslashes($pd['name']) ?>','<?= addslashes($pd['email']) ?>','<?= addslashes($pd['phone']) ?>','<?= addslashes($pd['location']) ?>','<?= addslashes($pd['jobTitle']) ?>','<?= addslashes($pd['description']) ?>')">View</button>
                                    <button class="btn btn-warning btn-sm" data-bs-toggle="modal" data-bs-target="#editModal"
                                        onclick="editDetail(<?= $pd['id'] ?>,'<?= addslashes($pd['name']) ?>','<?= addslashes($pd['email']) ?>','<?= addslashes($pd['phone']) ?>','<?= addslashes($pd['location']) ?>','<?= addslashes($pd['jobTitle']) ?>','<?= addslashes($pd['description']) ?>')">Edit</button>
                ...
                <form class="modal-content" method="post" action="">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-2">
                            ...
                            <input type="file" class="form-control" name="profilePhoto" required>
                        </div>
                    </div>
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <div><strong>Job title:</strong> <span id="viewJobTitle"></span></div>
                        <div><strong>Description:</strong> <span id="viewDescription"></span></div>
                    </div>
                    <div class="modal-footer">
            ...
            <div class="modal-dialog">
                <form class="modal-content" method="post" action="">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-2">
                            <label class="form-label">Location</label>
                            <input type="text" class="form-control" name="jobTitle" id="editjobTitle" required>
                        </div>
                        <div class="mb-2">
                            <label class="form-label">Location</label>
        ...
        <script>
        function viewDetail(name, email, phone, location, jobTitle, description) {
        ...
        function editDetail(id, name, email, phone, location, jobTitle, description) {
